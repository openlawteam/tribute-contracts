...
function internalTransfer(
  ...
  if (member == null) {
    ...
    // member.createdAt = event.block.timestamp.toString(); // @todo
...
export function handleNewBalance(event: NewBalance): void {
  ...
  internalTransfer(event.address, event.params.member, event.params.tokenAddr);
}
...
export function handleWithdraw(event: Withdraw): void {
  ...
  internalTransfer(event.address, event.params.account, event.params.tokenAddr);
}
