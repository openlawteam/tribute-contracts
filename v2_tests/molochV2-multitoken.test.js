...
const deploymentConfig = {
  ...
  'DILUTION_BOUND': 3,
  'PROCESSING_REWARD': 1,
  'TOKEN_SUPPLY': new BN("100000000000000000000")
}

async function moveForwardPeriods (periods) {
  await blockTime()
  const goToTime = deploymentConfig.PERIOD_DURATION_IN_SECONDS * periods
  await advancedCall('evm_increaseTime', [goToTime])
  await forceMine()
  await blockTime()
  return true
}
  ...
  describe('RAGEQUIT AND WITHDRAW TOKENS AT MAXIMUM TOKEN LIMITS', () => {
    ...
    beforeEach(async function() {
      ...
      await fundAndApproveToMoloch({
        ...
        to: summoner,
        from: creator,
        value: web3.utils.toBN(deploymentConfig.PROPOSAL_DEPOSIT).mul(MAX_TOKEN_WHITELIST_COUNT)
      })
