...
contract('MolochV3 - Managing Adapter', async accounts => {
  ...
  async function createDao(member, proposal, voting, ragequit, managing, financing, senderAccount) {
  ...
  it("should not be possible to propose a new module with 0x0 module address", async () => {
    ...
    //Create the new DAO
    let dao = await createDao(member, proposal, voting, ragequit, managing, financing, myAccount);

    //Submit a new Bank module proposal
    let newModuleId = Web3.sha3('bank');

    try {
      await managing.createModuleChangeRequest(dao.address, applicant, newModuleId, token);
    } catch (err) {
  ...
  it("should be possible to any individual to propose a new DAO Banking module", async () => {
    ...
    await managing.sponsorProposal(dao.address, proposalId, [], { from: myAccount, gasPrice: Web3.toBN("0") });
    await voting.submitVote(dao.address, proposalId, 1, { from: myAccount, gasPrice: Web3.toBN("0") });
    await advanceTime(10000);
    await managing.processProposal(dao.address, proposalId, { from: myAccount, gasPrice: Web3.toBN("0") });
